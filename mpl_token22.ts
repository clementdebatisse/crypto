...
async function main(){
    console.log("Changement des meta données du token !");
    const myKeypair = loadWalletKey("3gT6Tp8FkiE58VC8fxmLdT1GvmuZfinLDqPNsPPPimR4.json");
    const mint = new web3.PublicKey("bvnngKgh8b1uVZs1JjsZL64vH8JLALQjckiPCNyHBQe");

    const umi = createUmi("https://api.devnet.solana.com");
    const signer = createSignerFromKeypair(umi, fromWeb3JsKeypair(myKeypair))
    umi.use(signerIdentity(signer, true))

    const ourMetadata = { // TODO change these values for your token !
        name: "eXalt Token", 
        symbol: "EXALT",
